...
- **Decision Digest (optional):** When Next.js has `DECISION_DIGEST_ENABLED=true`, digest generation via `POST /api/scan` may persist extra fields on `digestRuns`: `deltaSummary`, `materialitySummary`, `strategicReadSummary`, `recommendedActionsSummary`, and `confidence` (`low` \| `medium` \| `high`). Older runs omit these fields. The digest detail page shows a **Decision brief** card when any are present. **sendDigestEmail** includes the same block in HTML when present (still after executive summary rules for team-filtered recipients).
