...
| R-DIG-10 | Optional feature | **If** the Next.js environment variable `DECISION_DIGEST_ENABLED` is set to `true`, **the system shall** request and persist optional Decision Digest fields on `digestRuns`: `deltaSummary`, `materialitySummary`, `strategicReadSummary`, `recommendedActionsSummary`, and `confidence` (`low` \| `medium` \| `high`) when digest generation runs via `POST /api/scan` with a Groq API key (`GROQ_API_KEY`); **if** it is unset or false, **the system shall** not require or persist those fields beyond existing executive summary and items. |
...
| R-DIG-13 | Optional feature | **If** Decision Digest fields are present on a digest run, **the system shall** include them in the Resend HTML digest email (for recipients who receive the executive summary) after the executive summary block. |
