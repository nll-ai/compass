...
| R-SCH-8 | Ubiquitous | **The system shall** on the Settings **Digest schedule** panel let each user set a Decision brief preference (`inherit` \| `enabled` \| `disabled`) for digest emails, persisted on their user profile, with default `inherit`. |
| R-SCH-9 | Ubiquitous | **The system shall** on the same **Digest schedule** panel let each user set **digest email source recency** (calendar days), persisted as `users.digestEmailLookbackDays`, choosing from the product’s fixed list (**7**, **14**, **30**, **60**, **90**), clamped server-side to **`1..365`**; when unset, the email path shall default to the same **`14`**-day default as scan/UI recency (`DEFAULT_LOOKBACK_DAYS`). |
...
| R-SCAN-3 | Event-driven | **When** **`POST /api/scan`** or **`POST /api/scan/pubmed`** throws after a scan run id is known, **the system shall** persist that run as **`failed`** (and close out per-source rows still `pending` or `running`) via a server-authenticated Convex mutation so the run does not remain **`running`** after the HTTP 500. |
...
| R-SCAN-12 | Ubiquitous | **The system shall** expose **`POST /api/scan/pubmed`** for programmatic PubMed-only scans of a single watch target (`watchTargetId`, optional `period`, optional `mode`, optional `pubmedPubDate` for PubMed publication-date window, optional **`lookbackDays`** per R-SRC-3), using the **same server authentication** as **`POST /api/scan`** (`SCAN_SECRET` / `Authorization: Bearer`) and the **shared scan pipeline** (see LLD §4.2.1). |
...
| R-SCAN-UI-2 | Ubiquitous | **The system shall** always request **`mode: "comprehensive"`** from UI-initiated scans (Watch Targets cards, target detail, Re-run) so retrieval uses the comprehensive source path. **The system shall** rely on the default **`lookbackDays`** window (**14** calendar days; optional **`0`** for no limit) on **`POST /api/scan`** to scope which new items are **upserted** and **summarized into digests** (R-SRC-3), and **shall not** expose a separate “quick vs deep” scan mode in the UI (see R-SCAN-UI-5). |
...
| R-DIG-4 | State-driven | **Where** `scanRuns.digestNotifyUserIds` is unset or empty, **the system shall** resolve the recipient email from the digest run’s scan run, then the first watch target in that run, then that target’s owning user’s email. |
...
| R-DIG-8 | Ubiquitous | **The system shall** send a **combined digest email** when a scan run covers multiple watch targets: HTML body with executive summary and optional Decision brief **only when at least one digest item remains for that recipient after applying that recipient’s digest-email source recency window** (see R-SCH-9); a top-level **signals in the last N days** section (**N** = recipient preference) **shall** include short copy stating that eligibility uses **linked-source publication or ingestion time**, not scan **`lookbackDays`** alone; **every in-scope watch target** in the run shall appear in a per-target section — either signal cards (significance + category pills, headline, full synthesis/fallback text, and source links) when any linked **`rawItems`** fall on or after the cutoff derived from **`digestRun.generatedAt`** and **N**, or a short **nothing new in the last N days** message when none qualify. Cutoff uses **`publishedAt ?? _creationTime`** on linked raw rows; digest items with **no** `rawItemIds` remain included. Links to `/targets/{id}/digests`, `/targets`, and external sources **shall** use validated `href` values, and rendered text fields **shall** be HTML-escaped. **When** `scanRuns.digestNotifyUserIds` is set, **the system shall** send one message per listed user (deduplicated by user id), filtering digest items to that user’s subscribed targets when the user has a `teamId`; **when** the user has **no** `teamId`, **the system shall** not apply that subscription filter (all targets in the run remain in scope for that recipient). |
...
| R-DIG-10 | Optional feature | **By default, the system shall** request and persist optional Decision Digest fields on `digestRuns` (`deltaSummary`, `materialitySummary`, `strategicReadSummary`, `recommendedActionsSummary`, and `confidence` (`low` \| `medium` \| `high`)) when digest generation runs via `POST /api/scan` with a Groq API key (`GROQ_API_KEY`). **If** Next.js env `DECISION_DIGEST_ENABLED` is explicitly set to `false`, **the system shall** disable that generation path. **When** `scanRuns.digestNotifyUserIds` exists, recipient-level preferences (`inherit` \| `enabled` \| `disabled`) **shall** override the system default for generation intent: generate Decision Digest fields when at least one intended recipient resolves to enabled. |
| R-DIG-11 | Ubiquitous | **The system shall** on the digest detail page (`/digest/{id}`) show a **Decision brief** section when any Decision Digest field is present, without breaking rendering for older digest runs that omit those fields. |
| R-DIG-12 | Ubiquitous | **The system shall** allow users who can view a digest item’s watch target to set `workflowStatus`, set `assigneeUserId` to an allowed user (target owner or same-team member), and add comments stored in `digestItemComments`. |
| R-DIG-13 | Optional feature | **If** Decision Digest fields are present on a digest run, **the system shall** include them in the Resend HTML digest email after the executive summary block **when** the recipient receives the executive summary **and** at least one signal remains after digest-email source recency filtering. Recipient-level Decision brief preference (`enabled` / `disabled`) **shall** override the system default; `inherit` uses the system default. |
...
| R-SRC-3 | Ubiquitous | **The system shall** default **Source Links** (`/targets/{id}`) and the **Timeline** (`/targets/{id}/timeline`) to a **14-calendar-day** recency window for listed raw items, with **segmented controls** on both views to switch to **7 days**, **30 days**, or **All** (no date filter), using URL query **`range`** (`7d`, `14d`, `30d`, `all`, or **`0`**) so filters are deep-linkable; **the system shall** use a **single shared preset list** for those **calendar-day** values (see LLD `LOOKBACK_UI_PRESET_DAYS`) and **shall** treat any other **`range`** value (including non-preset **`Nd`**) as the default **14**-day window. **The system shall** pass the resulting window to **`rawItems.listByWatchTarget`** as **`lookbackDays`** (**omitted argument** defaults to **14** in Convex; **`0`** = no date filter). **The system shall** apply **`publishedAt ?? _creationTime`** when filtering listed items and digest inputs. **The system shall** apply the same default and optional **`lookbackDays`** (**0**–**365** on scan routes; **`0`** = no recency cutoff) on **`POST /api/scan`** and **`POST /api/scan/pubmed`** for digest synthesis and inbound raw-item filtering when **> 0**, and shall narrow **PubMed `esearch`** to the last **N** calendar-day **`pdat`** range through today (UTC) when **`pubmedPubDate` is omitted** and **`lookbackDays` > 0**. **Where** **`pubmedPubDate` is omitted** and **`lookbackDays` is `0`**, **the system shall** use the default **contemporaneous** PubMed **`pdat`** window (**3** years through today, UTC) for **`esearch`**, not the last-**N**-days range. **The system shall** apply **server-side date filtering** on ClinicalTrials.gov API queries using the **`LastUpdatePostDate`** field (via `filter.advanced` `AREA[LastUpdatePostDate]RANGE[<mindate>,MAX]`) when **`lookbackDays` > 0**, so only recently updated trials are returned. **The system shall** extract `publishedAt` for ClinicalTrials.gov raw items from **`lastUpdatePostDateStruct`** (last posted update date) as the primary field, falling back to **`studyFirstPostDateStruct`** then **`startDateStruct`** when earlier fields are absent. **The system shall** drop inbound raw items **without** `publishedAt` when `lookbackDays > 0`, because the lack of a date prevents recency verification and the `_creationTime` fallback would make undated items appear falsely recent. |

## 4.6 FiercePharma news source

| ID | Pattern | Requirement |
|----|---------|-------------|
| R-FP-1 | Ubiquitous | **The system shall** include FiercePharma as a scan source (`source: "fiercepharma"`); **when** `EXA_API_KEY` is set, **the system shall** search FiercePharma via Exa restricted to `fiercepharma.com` (one query per watch target, `category: "news"`), and **otherwise shall** fall back to fetching the global RSS feed and keyword-matching by name/displayName/aliases/company/learnedQueryTerms (minus excludeQueryTerms). **The system shall** emit one raw item per matched target (per-target dedup, consistent with R-SCAN-11). |
| R-FP-2 | Optional feature | **If** `EXA_API_KEY` is set, **the system shall** apply `startPublishedDate` from the scan's `lookbackDays` for server-side recency and shall use Exa's returned article text (`contents.text`) and `publishedDate` without a separate page fetch. |
| R-FP-3 | State-driven | **Where** `lookbackDays` is greater than **0**, **the system shall** populate `publishedAt` on each FiercePharma item (from Exa `publishedDate` or the parsed feed `pubDate`) so the standard recency filter applies (items without a date are dropped), consistent with R-SRC-3. |
| R-FP-4 | Optional feature | **If** `GROQ_API_KEY` is set, **the system shall** derive target-tailored summaries from article content (Exa text, or a best-effort page fetch on the RSS fallback); **otherwise** it shall store an excerpt/feed lede and rely on the shared `enrichMissingSummaries` pass. |
| R-FP-5 | Unwanted behavior | **The system shall not** fail a scan when the Exa search, RSS feed fetch, or an individual article fetch errors; **the system shall** return collected items and surface a source-level error only when no items result. |
