...
| R-SCAN-12 | Ubiquitous | **The system shall** expose **`POST /api/scan/pubmed`** for programmatic PubMed-only scans of a single watch target (`watchTargetId`, optional `period`, optional `mode`, optional `pubmedPubDate` for PubMed publication-date window), using the **same server authentication** as **`POST /api/scan`** (`SCAN_SECRET` / `Authorization: Bearer`) and the **shared scan pipeline** (see LLD §4.2.1). |
...
| R-DIG-8 | Ubiquitous | **The system shall** send a **combined digest email** when a scan run covers multiple watch targets: HTML body with executive summary (omitted for team-filtered recipients when they have no matching digest items), a top-level signals count, per-target sections, and per-signal card blocks that mirror digest detail semantics (significance + category pills, headline, full synthesis/fallback text, and source links). Links to `/targets/{id}/digests`, `/targets`, and external sources **shall** use validated `href` values, and rendered text fields **shall** be HTML-escaped. **When** `scanRuns.digestNotifyUserIds` is set, **the system shall** send one message per listed user (deduplicated by user id), filtering digest items to that user’s subscribed targets when the user has a `teamId`. |
...
| R-DIG-13 | Optional feature | **If** Decision Digest fields are present on a digest run, **the system shall** include them in the Resend HTML digest email (for recipients who receive the executive summary) after the executive summary block. Recipient-level Decision brief preference (`enabled` / `disabled`) **shall** override the system default; `inherit` uses the system default. |
