...
| `lib/kg/backbones/{uniprot,ensembl,opentargets,chembl}.ts` | Open backbone clients (REST/GraphQL via `fetchWithRetry`); CC-BY; return typed normalized records (UniProt target normalize, Ensembl symbol→ENSG, Open Targets associations, ChEMBL drug mechanisms). |
...
- **Request body:** `scanRunId?`, `period` ("daily" | "weekly"), `targetIds?`, `mode?` ("latest" | "comprehensive"), `sources?`, `pubmedPubDate?`, **`lookbackDays?`** (number; **`0`** = no recency limit, omitted → **14** calendar days).
  ...
  - `lookbackDays` (optional, all sources): When **> 0**, the pipeline (1) drops inbound raw rows whose **`publishedAt`** is before the cutoff (**rows without `publishedAt` are also dropped** — lack of date means recency cannot be confirmed), (2) passes only new rows in that window into digest synthesis (`publishedAt` with fallback to **`_creationTime`**), and (3) when **`pubmedPubDate` is omitted**, sets PubMed `esearch` to a **`pdat`** **range** from **today − N days** through **today** (UTC) instead of the legacy **3-year contemporaneous** default; ClinicalTrials.gov queries add a **`filter.advanced`** clause **`AREA[LastUpdatePostDate]RANGE[<mindate>,MAX]`** to exclude stale trials server-side. When **`0`**, PubMed date behavior for an omitted **`pubmedPubDate`** reverts to **contemporaneous 3 years**; ClinicalTrials.gov queries omit the date filter; digest and upsert do not apply a recency cutoff.
...
- **FiercePharma:** Domain-scoped Exa search source. **Primary** (`EXA_API_KEY` set): one Exa `/search` per watch target with `includeDomains: ["fiercepharma.com"]`, `category: "news"`, `startPublishedDate` from `lookbackDays`, returning `contents.text` and `publishedDate`; emits one raw item per matched target, with a target-tailored summary from the returned text when `GROQ_API_KEY` is set (else excerpt → shared `enrichMissingSummaries`). **Fallback** (no `EXA_API_KEY`): fetches the global RSS feed and keyword-matches title/description to targets (feed `pubDate` → `publishedAt`; best-effort page fetch for summaries, else feed lede). Same downstream `filterRelevantItems` LLM precision pass as other sources. Runs in the default source set. Files: `lib/scan/sources/fiercepharma.ts`, `lib/scan/sources/fiercepharma-agent.ts`.
...
- **rawItems `source` union:** `pubmed` | `biorxiv` | `clinicaltrials` | `edgar` | `exa` | `openfda` | `rss` | `patents` | **`fiercepharma`** (see `convex/schema.ts`); adding a source requires a new literal **both** in the `rawItems.source` schema union **and** in the duplicate `sourceValidator` union in `convex/rawItems.ts` (it backs the `source` arg of `insertRawItem` / `upsertRawItemsFromServer` / `getByExternalId*`), plus a registry entry in `lib/sources/registry.ts` and a runner in `lib/scan/sources/index.ts` and `lib/scan/orchestrator.ts`.
...
- **Knowledge graph backbones:** no API keys required (UniProt REST, Ensembl REST, Open Targets Platform GraphQL, and ChEMBL REST are unauthenticated and CC-BY); company normalization reuses **`SEC_EDGAR_USER_AGENT`** via `resolveCompanyToSEC`.
